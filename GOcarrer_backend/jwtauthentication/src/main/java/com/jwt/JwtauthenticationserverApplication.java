...
import com.jwt.repo.RecruiterRepository;
...
public class JwtauthenticationserverApplication implements  CommandLineRunner  {
    ...
    public void createRecruiter() {
         ...
         Long id = new Long(random.nextInt(100));
         rec.setId(id);
         rec.setEmail("user" + id + "@gmail.com");
         rec.setPassword("user" + id);
         rec.setEnabled(true);
         rec.setRol("Admin");
         rec.setUsername("user" + id);
         Recruiter  save = this.recruiterRepository.save(rec);
    }
